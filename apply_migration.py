#!/usr/bin/env python3
"""
Direct migration script that can be run from any directory.
...
async def apply_migration():
    """Apply marketplace migration directly."""
    
    # Split migration into smaller chunks to avoid issues
    migrations = [
        ...
        async with session_factory() as session:
            ...
            for i, sql in enumerate(migrations, 1):
                try:
                    ...
                    print(f"✅ {i}/{len(migrations)}")
                except Exception as e:
                    print(f"⚠️  {i}/{len(migrations)}: {str(e)[:100]}")
