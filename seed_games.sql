-- Seed games, categories and products via raw SQL
-- This bypasses SQLAlchemy enum issues
...
-- Insert categories for other popular games
INSERT INTO categories (game_id, slug, title, is_active, sort_order, is_deleted, created_at, updated_at)
SELECT id, 'gold', 'Золото', true, 1, false, NOW(), NOW() FROM games WHERE slug = 'standoff-2'
ON CONFLICT DO NOTHING;

INSERT INTO categories (game_id, slug, title, is_active, sort_order, is_deleted, created_at, updated_at)
SELECT id, 'accounts', 'Аккаунты', true, 2, false, NOW(), NOW() FROM games WHERE slug = 'standoff-2'
ON CONFLICT DO NOTHING;
...
INSERT INTO categories (game_id, slug, title, is_active, sort_order, is_deleted, created_at, updated_at)
SELECT id, 'accounts', 'Аккаунты', true, 2, false, NOW(), NOW() FROM games WHERE slug = 'roblox'
ON CONFLICT DO NOTHING;
...
-- Insert sample products for Brawl Stars gems
...
WHERE g.slug = 'brawl-stars' AND c.slug = 'gems'
ON CONFLICT DO NOTHING;
...
FROM games g
JOIN categories c ON c.game_id = g.id
WHERE g.slug = 'roblox' AND c.slug = 'robux'
ON CONFLICT DO NOTHING;
...
-- Add products for Genshin Impact
INSERT INTO products (game_id, category_id, slug, title, is_active, is_featured, sort_order, is_deleted, fulfillment_type, requires_player_id, requires_nickname, requires_region, requires_manual_review, requires_screenshot, extra_fields_schema_json, created_at, updated_at)
SELECT 
    g.id,
    c.id,
    'genshin-primogems-60',
    '60 Примогемов',
    true,
    false,
    1,
...
FROM games g
JOIN categories c ON c.game_id = g.id
WHERE g.slug = 'genshin-impact' AND c.slug = 'primogems'
ON CONFLICT DO NOTHING;
