...
async def promo_view(callback: CallbackQuery, session: AsyncSession) -> None:
    settings = SettingsService(session)
    text = await settings.get(
        "promo_help_text",
        "🎁 <b>Промокоды</b>\n\nСейчас отдельный экран ввода промокода ещё не реализован. "
        "Если у вас есть код, передайте его оператору поддержки или добавьте его в следующей версии отдельным шагом оформления.",
    )
    await callback.message.edit_text(text, parse_mode="HTML")
    await callback.answer()
