...
async def choose_manual_payment(
    ...
    text = (
        ...
        f"Сумма: <b>{order.total_amount} {order.currency_code}</b>\n\n"
        f"Для оплаты свяжитесь с поддержкой магазина.\n"
        f"После подтверждения платежа администратор переведёт заказ в обработку."
    )
...
@router.callback_query(PaymentCb.filter(F.action.in_(["stars", "cryptobot"])))
async def payment_method_not_available(
    callback: CallbackQuery,
) -> None:
    await callback.answer("Этот способ оплаты пока не подключён", show_alert=True)
