...
def payment_methods_kb(
    ...
    *,
    ...
    if stars_enabled:
        b.button(
            text="⭐ Оплатить Stars",
            callback_data=PaymentCb(action="stars", order_id=order_id).pack(),
        )

    if cryptobot_enabled:
        b.button(
            text="💎 Оплатить Crypto Bot",
            callback_data=PaymentCb(action="cryptobot", order_id=order_id).pack(),
        )

    b.button(
        text="💬 Ручная оплата",
        callback_data=PaymentCb(action="manual", order_id=order_id).pack(),
    )
        ...
        inline_keyboard=[
            ...
            [
                InlineKeyboardButton(
                    ...
                    callback_data=PaymentCb(action="cryptocheck", order_id=order_id).pack(),
                )
            ],
