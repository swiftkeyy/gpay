...
def games_admin_kb(games: list) -> InlineKeyboardMarkup:
    ...
    for game in games:
        b.button(
            text=f"{_status_mark(getattr(game, 'is_active', False))} {getattr(game, 'title', 'Без названия')}",
...
def categories_admin_kb(categories: list, *, game_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        game_title = getattr(getattr(category, "game", None), "title", f"game:{getattr(category, 'game_id', '-')}")
        b.button(
            text=f"{_status_mark(getattr(category, 'is_active', False))} {getattr(category, 'title', 'Без названия')} · {game_title}",
...
def products_admin_kb(products: list, *, category_id: int | None = None) -> InlineKeyboardMarkup:
    ...
    for product in products:
        category_title = getattr(getattr(product, "category", None), "title", f"cat:{getattr(product, 'category_id', '-')}")
        b.button(
            text=f"{_status_mark(getattr(product, 'is_active', False))} {getattr(product, 'title', 'Без названия')} · {category_title}",
...
def prices_admin_kb(rows: list[tuple]) -> InlineKeyboardMarkup:
    ...
    for product, price in rows:
        ...
        b.button(
            text=f"💸 {getattr(product, 'title', 'Без названия')} · {amount}",
...
def confirm_keyboard(confirm_value: str, cancel_value: str = "cancel") -> InlineKeyboardMarkup:
    b = InlineKeyboardBuilder()
    b.row(
        InlineKeyboardButton(text="✅ Подтвердить", callback_data=confirm_value),
        InlineKeyboardButton(text="❌ Отмена", callback_data=cancel_value),
    )
    return b.as_markup()
